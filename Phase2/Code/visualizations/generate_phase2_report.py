...
MODEL_PATH = "/home/adipat/Documents/Spring_26/CV/P1/Deep_homography/Deep-Homography/model_weights/unsupervised_homography_net_mod(1).pth"
...
class HomographyNet(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(nn.Linear(128*16*16, 1024), nn.ReLU(True), nn.Linear(1024, 8))
...
class UnifiedHomographyModel(nn.Module):
    ...
    def forward(self, stacked, rho=32):
        ...
        if self.model_type == "Sup": 
            ...
            # Unsupervised Logic: Tanh activation + Scaling
...
def run_epe_benchmark(name, model, folder_path, num_samples=200):
    ...
    with torch.no_grad():
        for _ in range(num_samples):
            ...
            start = time.time()
            # Note: Unsup logic is handled inside model forward
...
def main():
    ...
    # Initialize with "Unsup" so the Forward pass uses Tanh
    ...
    print("\n--- 3. GENERATING VISUALIZATIONS (Fig 18 - Unsup) ---")
    ...
    plt.tight_layout()
    plt.savefig("Fig18_Comparisons_Unsup.png")
    print("Saved 'Fig18_Comparisons_Unsup.png'")

    print("\n--- 4. QUANTITATIVE RESULTS (EPE & RUNTIME) ---")
    print("-" * 60)
